...
import { createMetaImage, createSocialMetaImage } from './create-meta-image.js';
...
export default function clueOfTheDay(cotd) {
    ...
    const allClues = Object.keys(cotd.clues)
        .map((date, index) => {
            ...
            const author = decodedClue.author?.trim();

            const friendlyDate = new Date(date).toLocaleDateString('en-GB', {
                month: 'short',
                day: 'numeric',
                year: 'numeric',
            });
    ...
    return function cotd(files, metalsmith) {
        ...
        allClues
            .filter((clue) => clue.date >= day)
            .filter((_) => !process.env.SKIP_META_IMAGES)
            .forEach(async (cotd) => {
        ...
        files['index.html'].cotd = todayAndPreviousCotd.slice(0, 4);
