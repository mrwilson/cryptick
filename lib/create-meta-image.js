...
export async function createMetaImage(clue) {
    ...
    const lines = clue.clue.match(/.{1,25}(?:\s|$)/g);

    const fontSize = config.clue.fontSize * (lines.length >= 5 ? 0.8 : 1);
    ...
    lines.forEach((line, index) => {
        context.fillText(
            line,
            50,
            (630 - fontSize * lines.length) / 2 + fontSize * index,
        );
    });
...
export async function createSocialMetaImage(clue) {
    ...
    context.fillText(`Cryptick | Clue of the Day #${clue.id}`, 40, 40);
    context.fillText(`${clue.friendlyDate}`, 40, config.size.y - 100);
    context.fillText(`Author: ${clue.author_name}`, 40, config.size.y - 180);

    const lines = clue.clue.match(/.{1,15}(?:\s|$)/g);

    const fontSize =
        config.clue.fontSize *
        (lines.length >= 5 || Math.max(...lines.map((x) => x.length)) >= 12
            ? 0.9
            : 1);
    ...
    lines.forEach((line, index) => {
        context.fillText(
            line,
            50,
            (config.size.y - fontSize * lines.length) / 2 + fontSize * index,
        );
    });
